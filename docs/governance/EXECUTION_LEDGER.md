...
| LEDGER-0020 | TASK-0501 | 梳理 Builder Code 注入约束并接入最小透传骨架 | Manus AI | in_progress | TASK-0406 完成 | 官方文档核对、代码审查与 `pnpm check` | 已核对 X Layer 官方文档，确认 Builder Code 需要在**实际发送交易的客户端**通过 `dataSuffix` 注入，推荐落点是 Viem / Wagmi 钱包客户端；当前仓库仍无本地 `sendTransaction` 或 `wallet_sendCalls` 实现，真实发送仍由 Onchain / DEX 执行接口与外部确认页承接，因此暂不具备完全本地注入条件。本轮已新增 `lib/builder-code.ts`，统一读取 `EXPO_PUBLIC_XLAYER_BUILDER_CODE`；已将 Builder Code 元信息写入 `PendingSignatureContext` 与聊天页外部确认页唤起参数，同时在 Onchain 配置快照中增加 `builderCodeConfigured` 状态，并更新 `.env.example`。在此基础上，本轮进一步依据 Agent Wallet、DEX Swap 与 Builder Code 钱包提供方文档，把 `builderCode / dataSuffix / callDataMemo` 从前端待确认上下文继续下沉到 DEX 执行主路径：`server/_core/dex-swap.ts` 已支持根据 X Layer Builder Code 生成 `dataSuffix` 与固定长度 `callDataMemo`，并在 OKX `/dex/aggregator/swap` 查询中附加 `callDataMemo`，同时把后缀追加到待签名 `swapTransaction.tx.data`（若存在）并通过 `builderCodeContext` 回显 `appliedToSwapQuery / appliedToPreparedTransaction`；聊天页与签名桥接类型也已同步承载 `builderCodeDataSuffix / builderCodeCallDataMemo`，当前 `pnpm -s tsc --noEmit` 通过。剩余缺口已收敛为：需要 App 内 Agent Wallet 真实发送层真正消费这些字段并完成 `wallet_sendCalls / sendTransaction` 实盘验证。 |
