...
| LEDGER-0015 | TASK-0402 | 评估真实签名桥接层缺口 | Manus AI | in_progress | 兑换执行进度与回执接入完成 | 代码检索、依赖检查与类型检查 | 已确认服务端具备 Swap 报价、广播与订单查询能力，但客户端当前尚未引入钱包签名桥接相关依赖与 signedTx 回传路径；目前已先完成转账结果卡片的前端承接层，并新增签名桥接基础模块 `lib/signature-bridge.ts`，提供待续跑执行上下文存储与回调参数解析能力，当前类型检查通过；下一步需补对话主线程到签名结果回传页面、桥接与广播升级路径 |
