...
| LEDGER-0015 | TASK-0402 | 评估真实签名桥接层缺口 | Manus AI | in_progress | 兑换执行进度与回执接入完成 | 代码检索与依赖检查 | 已确认服务端具备 Swap 报价、广播与订单查询能力，但客户端当前尚未引入钱包签名桥接相关依赖与 signedTx 回传路径；目前已先完成转账结果卡片的前端承接层，下一步需补对话主线程到签名结果回传的桥接与广播升级路径 |
