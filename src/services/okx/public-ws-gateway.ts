import { getDefaultOkxPublicSubscriptions, getOkxPublicWsUrl } from "./config";
import {
...
/**
 * OKX 公共 WebSocket 网关。
 * 负责 BTC / ETH 行情订阅、心跳保活、异常关闭后的自动重连与重订阅。
 */
export class OkxPublicWsGateway {
  ...
  constructor(options?: OkxPublicWsGatewayOptions) {
    this.url = options?.url ?? getOkxPublicWsUrl();
    ...
    getDefaultOkxPublicSubscriptions().forEach((arg) => {
  ...
  connect() {
    ...
    this.logger?.info?.("正在连接 OKX 公共 WebSocket", { url: this.url });
  ...
  disconnect(reason = "manual-close") {
    this.logger?.info?.("主动关闭 OKX 公共 WebSocket", { reason });
  ...
  private bindSocketEvents(socket: WebSocket) {
    socket.onopen = () => {
      this.logger?.info?.("OKX 公共 WebSocket 已连接");
    ...
    socket.onmessage = (event) => {
      ...
      try {
        ...
        this.logger?.warn?.("解析 WebSocket 消息失败", {
    ...
    socket.onerror = (event) => {
      this.logger?.error?.("OKX 公共 WebSocket 发生错误", {
        event: typeof event === "object" ? JSON.stringify(event) : String(event),
      });
    };

    socket.onclose = (event) => {
      this.logger?.warn?.("OKX 公共 WebSocket 已关闭", {
        code: event.code,
        reason: event.reason,
      });
      this.stopHeartbeat();
  ...
  private startHeartbeat() {
    ...
    this.heartbeatTimer = setInterval(() => {
      ...
      if (this.awaitingPong) {
        this.logger?.warn?.("上一个 ping 未收到 pong，准备重连");
        this.ws.close();
        return;
      }
      ...
      this.ws.send("ping");
      this.logger?.debug?.("发送 OKX ping");
      this.pongTimer = setTimeout(() => {
        if (!this.awaitingPong) {
          return;
        }
        this.logger?.warn?.("pong 超时，准备重连");
        this.ws?.close();
      }, this.pongTimeoutMs);
  ...
  private scheduleReconnect() {
    ...
    this.logger?.warn?.("准备重连 OKX 公共 WebSocket", { attempt: this.reconnectAttempts, delay });
