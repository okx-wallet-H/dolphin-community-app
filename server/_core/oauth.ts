...
export function registerOAuthRoutes(app: Express) {
  app.get("/api/oauth/callback", async (req: Request, res: Response) => {
    ...
    try {
      ...
      // 重定向到前端 URL（Expo web 在端口 8081）
      // Cookie 使用父域设置，因此可在 3000 和 8081 子域中工作
      const frontendUrl =
  ...
  // 从 Bearer token 建立会话 cookie
  // 由 iframe 预览使用：前端通过 postMessage 接收 token，然后调用此端点
  // 从后端（3000-xxx 域）获取适当的 Set-Cookie 响应
  app.post("/api/auth/session", async (req: Request, res: Response) => {
    try {
      // 使用来自 Authorization 标头的 Bearer token 进行身份验证
      const user = await sdk.authenticateRequest(req);

      // 从 Authorization 标头获取 token 以设置为 cookie
      const authHeader = req.headers.authorization || req.headers.Authorization;
