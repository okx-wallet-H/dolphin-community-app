...
- [ ] Builder Code 注入真实 X Layer 发送层（已完成 `EXPO_PUBLIC_XLAYER_BUILDER_CODE` 配置，并把 `builderCode / dataSuffix / callDataMemo` 下沉到 swap 查询与待签名交易层；待 App 内 Agent Wallet 真实发送链路消费这些字段并完成 `sendTransaction / wallet_sendCalls` 实盘验证）
- [x] 聪明钱追踪（大户动向）
