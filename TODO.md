...
- [ ] Onchain 执行风控规则（待补金额上限、白名单链、异常滑点拦截）
- [ ] Onchain 幂等与防重复提交（待补 idempotency key 与时间窗口校验）
- [ ] Serverless Onchain 路由生产化对齐（待同步持久化、风控与幂等逻辑）
- [ ] Builder Code 注入真实 X Layer 发送层（待接入真实 `sendTransaction/dataSuffix` 落点）
- [x] 聪明钱追踪（大户动向）
